...
class TestAPIPerformance:
    ...
    def test_single_request_performance(self, api_endpoints, test_timeout):
        ...
        for endpoint_name, url in endpoints:
            ...
            assert (
                response_time < 10.0
            ), f"{endpoint_name} response time too high: {response_time:.2f}s"
    ...
    def test_concurrent_requests(self, api_endpoints, performance_config, test_timeout):
        ...
        assert (
            success_rate >= performance_config["min_success_rate"]
    ...
    def test_message_size_performance(self, api_endpoints, test_timeout):
        ...
        for size in message_sizes:
            ...
            max_expected_time = 5.0 + (
                size / 1000
            )  # Allow more time for larger messages
            assert (
    ...
    def test_sustained_load(self, api_endpoints, performance_config, test_timeout):
        ...
        print(
            f"Running sustained load test for {duration}s at {requests_per_second} req/s..."
        )
        ...
        while time.time() - start_time < duration:
            try:
                ...
                results.append({"error": str(e), "timestamp": time.time()})
    ...
    def test_cold_start_performance(
        self, api_endpoints, lambda_info, aws_clients, test_timeout
    ):
        ...
        if total_time > 5.0:  # Only check if first request was likely a cold start
            assert (
                warm_time < total_time / 2
            ), "Warm request not significantly faster than cold start"
    ...
    def test_error_response_performance(self, api_endpoints, test_timeout):
        ...
        assert (
            response_time < 5.0
        ), f"Error response time too high: {response_time:.2f}s"
