...
class TestDeployedPerformance:
    ...
    def test_deployed_response_times(self, deployed_api_url):
        ...
        for rt in response_times:
            print(f"  {rt['endpoint']}: {rt['actual_time']:.2f}ms (header: {rt['header_time']:.2f}ms)")
        
        # Verify reasonable response times (allowing for cold starts)
        for rt in response_times:
            assert rt['actual_time'] <= 30000, f"Response time too high for {rt['endpoint']}: {rt['actual_time']}ms"
