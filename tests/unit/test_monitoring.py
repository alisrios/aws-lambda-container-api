...
class TestStructuredLogging:
    ...
    def test_structured_logging_format(self, caplog):
        ...
        with app.test_client() as client:
            ...
            log_records = [record for record in caplog.records if record.name == 'src.app']
            assert len(log_records) > 0
            
            # Verify log contains expected fields
            for record in log_records:
                if hasattr(record, 'extra_fields'):
                    assert 'endpoint' in record.extra_fields or 'method' in record.extra_fields
...
class TestLoggingIntegration:
    ...
    def test_before_request_logging(self, caplog):
        ...
        with app.test_client() as client:
            ...
            request_logs = [record for record in info_records if 'Incoming request' in record.message]
    ...
    def test_after_request_logging(self, caplog):
        ...
        with app.test_client() as client:
            ...
            info_records = [record for record in caplog.records if record.levelname == 'INFO']
            completion_logs = [record for record in info_records if 'Request completed' in record.message]
    ...
    def test_endpoint_specific_logging(self, caplog):
        ...
        with app.test_client() as client:
            ...
            info_records = [record for record in caplog.records if record.levelname == 'INFO']
            endpoint_logs = [record for record in info_records if 'Echo endpoint processed successfully' in record.message]
