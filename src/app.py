...
def hello():
    ...
    try:
        ...
        logger.info("Hello endpoint processed successfully", extra={
            'extra_fields': {
                'endpoint': '/hello',
        ...
        logger.error("Error in hello endpoint", extra={
            'extra_fields': {
                'endpoint': '/hello',
                'error': str(e),
...
def echo():
    ...
    try:
        ...
        if not msg:
            logger.warning("Echo endpoint accessed without required parameter", extra={
                'extra_fields': {
                    'endpoint': '/echo',
        ...
        logger.info("Echo endpoint processed successfully", extra={
            'extra_fields': {
                'endpoint': '/echo',
                'message_length': len(msg),
                'echoed_message': msg[:100] + '...' if len(msg) > 100 else msg  # Truncate long messages in logs
            }
        })
        ...
        logger.error("Error in echo endpoint", extra={
            'extra_fields': {
                'endpoint': '/echo',
                'error': str(e),
                'error_type': type(e).__name__,
...
def health():
    ...
    try:
        ...
        logger.error("Error in health endpoint", extra={
            'extra_fields': {
                'endpoint': '/health',
                'error': str(e),
                'error_type': type(e).__name__
